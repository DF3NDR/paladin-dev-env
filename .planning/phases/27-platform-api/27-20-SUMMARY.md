...
- **`REQUIREMENTS.md` PLAT-01 deliberately left unchecked by this plan.** This plan's frontmatter lists `requirements: [PLAT-01]`, but PLAT-01 is the broad "run submission decoupled from execution" requirement and plan 27-25 (`requirements: [PLAT-01, PLAT-02, PLAT-03, PLAT-04, PLAT-05, PLAT-06]`) is the actual closing/verification plan for the whole gap-closure set, gated on the CI `postgres-integration` job this plan cannot run locally (D-51). Marking PLAT-01 complete here would be premature ahead of that CI evidence; leaving it to 27-25 avoids a false-positive completion record.
