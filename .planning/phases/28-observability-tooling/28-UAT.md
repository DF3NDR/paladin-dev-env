...
updated: 2026-09-09T13:32:06Z
---

## Current Test

number: 1
name: Adjudicate the ≤3% tracing-overhead acceptance bar (PRD 07 criterion 6, D-37) against the measured FAIL
expected: |
  A maintainer decision recorded as a WINDOWS.md entry (or ADR amendment) for the
  measured result in 28-BENCH-EVIDENCE.md (log_sink +22.18%, composite +18.46% vs the
  untraced baseline). One of: (a) accept the deviation for v0.10.0 and close it as a
  documented WINDOWS.md item; (b) re-scope the bar to an I/O-bound (LLM-call) superstep
  rather than the synthetic all-Function-node microbenchmark; (c) require follow-up
  optimization of TraceDispatcher::emit / LogTraceSink serialization before shipping.
  The measurement and its FAIL verdict are already recorded (28-BENCH-EVIDENCE.md,
  28-06-SUMMARY.md, docs/src/operations/observability.md Known Limitations); what is
  missing is the close-out adjudication D-37 calls for and the WINDOWS.md entry
  (only #33/#34 exist for this phase).
