...
**BUG-04 (found 2026-09-03 by code reading during Phase 22.1):** *Resume rebuilt the Frontier from scratch, losing pre-crash edge resolutions.* `WarEngine::resume_with_options` hands `superstep::run` the Waypoint's Battlefield, Vanguard and visit counts, and `run` called `Frontier::new(graph)` — every edge `Pending`, `last_executed` empty — so per-edge resolutions recorded before the interruption were lost. The observable consequence: a pre-crash fired edge into a join node that was not yet ready is never seen again, so the resumed run can report `Completed` without a node the uninterrupted run executes, and ENG-04 / E2E-1's stated property ("final state equals the uninterrupted control run, zero re-execution") held only for shapes without such a pending join across the crash point. BUG-04 joins its siblings on the same truthful-outcome contract by a third mechanism: BUG-02 lied about a node no path could reach, BUG-03 about a node the readiness rule could never admit, and this one about a node whose admission ticket — a per-edge resolution already earned before the crash — was thrown away at the resume boundary. Fixed by ENG-FR-12a: a serializable `FrontierSnapshot` persisted on the `Waypoint`, keyed by edge identity (`from`, `to`, canonical condition) rather than edge index — so a resume under `allow_graph_change` degrades gracefully, dropping a snapshot edge the new graph no longer declares and starting a new edge unresolved rather than mis-assigning a stale resolution — seeded into the `Frontier` on resume, landed test-first. The `waypoint` module and the `engine` module are both absent at the `v0.9.0` tag, so this is a pre-release engine fix, **not** a v0.9 behavioral change: no `MIGRATION.md` §9.1 entry, no §9.2 X-10 register row, and no §9.4 persistence entry is required.
