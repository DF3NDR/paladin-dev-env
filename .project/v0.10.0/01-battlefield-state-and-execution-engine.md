...
- **ENG-FR-11 clarification (mid-muster progress Waypoints — CF-FR-12, D-14, Plan 23-06).** ENG-FR-11's "exactly one Waypoint after every superstep merge" remains true unchanged: a Muster's own superstep still produces exactly one superstep-COMPLETE Waypoint, written after that superstep's merge, exactly as any other superstep's. What is additive is that a Muster MAY ALSO write zero-or-more PROGRESS Waypoints INSIDE its own superstep, one per completed task (bounded by `EngineLimits::max_muster_tasks`) as tasks resolve, each carrying that superstep's own index with `status: Running`. A progress Waypoint's `battlefield` is the superstep-START snapshot, unchanged — never a partially merged state — and its `muster_progress` field carries the muster's full task list plus every completed task's UNMERGED delta, keyed by `task_key`; these are recovery records, not state transitions, and never substitute for the one superstep-complete Waypoint ENG-FR-11 already guarantees. A crash between two progress Waypoints therefore still resumes correctly: `resume` re-enters the SAME superstep (not the next one) and dispatches only the unfinished tasks, merging every delta — restored plus newly produced — in `task_key` order exactly once. E2E-1 contains no Muster and its one-Waypoint-per-superstep assertion is unaffected by this clarification.
...
- **ENG-FR-14 (Graph fingerprint).** `WarGraph` exposes a stable content fingerprint (hash over node ids plus each Paladin node's `output_field`, edge specs, the declared entry set, `defer_flags`, `dynamic_targets`, and each schema field's name plus its `DispatchRule` — NOT over prompts/models, which may be hot-swapped). Coverage of the entry set, `defer_flags`, and `dynamic_targets` was closed by Phase 22's CR-01 (22-REVIEW.md); the canonical byte encoding was subsequently re-done as a length-prefixed, collision-free scheme (`GRAPH_FINGERPRINT_VERSION` bumped to `v2`) by Phase 22.1's CR-01 (22.1-REVIEW.md) after a delimiter-based collision was found in the same function — see `WarGraph::fingerprint`'s rustdoc in `crates/paladin-battalion/src/engine/graph.rs` for the exact hashed-property list and encoding. The fingerprint is stored in each Waypoint; `resume` with a mismatched fingerprint → `EngineError::GraphMismatch { expected, got }` unless the caller passes an explicit `allow_graph_change: true` option (in which case unknown vanguard NodeIds fail with `UnknownField`-style precision: `EngineError::VanguardNodeMissing { node }`).
