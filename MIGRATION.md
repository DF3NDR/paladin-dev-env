...
- **New table: `waypoints`. Landing this plan (22-06).** Versioned migration file `crates/paladin-storage/migrations/sqlite/001_create_waypoints_table.sql` (this supersedes this section's earlier single-path placeholder from Plan 22-04 — the file lives under a per-backend `sqlite/` subdirectory, not directly in `migrations/`, since this crate is about to ship a second dialect of the same logical table for PostgreSQL; `001_` continues the per-crate numbering convention established by `crates/paladin-memory/migrations/001_create_garrison_tables.sql`). SQLite stores `payload` as `TEXT` holding serialized JSON (D-02, for debuggability and because the `sqlx` `json` feature is already enabled workspace-wide) and `status` as its own JSON-serialized column (so `history`/`list_threads` summaries can be built without deserializing the full payload). Schema: `waypoint_id` (primary key), `thread_id`, `parent_id` (nullable), `superstep`, `status`, `payload`, `created_at`, with an index on `(thread_id, created_at DESC)`. The PostgreSQL sibling migration (`.../postgres/001_create_waypoints_table.sql`, same logical schema with `payload JSONB`) lands in this same plan's Task 2 — see the entry directly below this one once that task completes.
- **Runs automatically, not manually.** The migration is embedded at compile time via `sqlx::migrate!` (which required adding `sqlx`'s `macros` feature to the workspace-level `sqlx` dependency — a Task 1 blocking-issue fix, no new crate name, see §9.3) and applied automatically when `SqliteWaypointStore` is constructed — no manual `sqlx migrate run` step is required of the operator. Idempotent: constructing a store twice against the same database file is safe (`CREATE TABLE IF NOT EXISTS`/`CREATE INDEX IF NOT EXISTS` plus `sqlx::migrate::Migrator`'s own applied-version tracking), proven by `constructing_store_twice_against_same_file_is_idempotent`.
