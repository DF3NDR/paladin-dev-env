...
//!
//! # Why this lives in `paladin-ports`, not the facade (deviation, D-20/D-21)
//!
//! `ConfinedVault` needs nothing beyond [`VaultPort`] (this crate) and
//! [`Namespace`] (`paladin-core`, already re-exported here) — no adapter,
//! no infrastructure dependency. Placing it here, beside the trait it
//! implements, is what makes it reachable from BOTH sides of Doc 05 RT-04:
//! `PaladinExecutionService` (the facade crate, `paladin-ai`) resolves and
//! enforces a run's grant through it, and `paladin-battalion`'s
//! `WarEngine`/`NodeContext` (Task 3 of this plan) grant a base namespace
//! to every node through it too. `paladin-battalion` depends on
//! `paladin-core` and `paladin-ports` ONLY — never on the facade crate,
//! which depends on `paladin-battalion` itself — so a type the engine's own
//! `NodeContext` must hold cannot live in the facade without an illegal
//! reverse dependency. The facade re-exports this type from its own
//! `application::services::paladin::vault_confined` path (and its prelude)
//! for source compatibility with how this plan was originally sketched.
//!
